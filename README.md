...
methods (such as logistic regression, SVM etc.) are not suitable for the application directly. We followed the idea from Poisson Binomial Distribution (PBD) model with normal 
...
we utilized BFGS and Newton's method to compute the estimator. More details could be found in .
